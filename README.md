...
```
````
The above chunk can be linked to `[like this](#importantChunk)`.

Figures with captions get an additional anchor. This can be linked
using the chunk label with a *_figure* suffix.

````r
```{r carsPlot, fig.cap="Figure 1: Plot of the cars dataset."}
plot(cars)
```
````

The figure generated by this code can then be linked to in the text
as `[Figure 1](#carsPlot_figure)`.
